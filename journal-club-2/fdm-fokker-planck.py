...
def dderivatives_grad(f, L, dr=1):
    ...
    for i in range(1, L[0] - 1):
        for j in range(1, L[1] - 1):
            if i==1:
                grad_x[i, j] = (f[1, j] - f[0, j]) / (2 * dr)
            else:
                grad_x[i, j] = (f[i + 1, j] - f[i, j]) / (2 * dr)

            if j==1:
                grad_y[i, j] = (f[i, 1] - f[i, 0]) / (2 * dr)
            else:
...
def dderivatives_div(fx, fy, L, dr=1):
    ...
    for i in range(1, L[0] - 1):
        for j in range(1, L[1] - 1):
            if i == 1 and j>1:
                div[i, j] = (fx[1, j] - fx[0, j]) / (2 * dr) + (fy[i, j + 1] - fy[i, j]) / (2 * dr)
            elif j==1 and i>1:
                div[i, j] = (fx[i + 1, j] - fx[i, j]) / (2 * dr) + (fy[i, 1] - fy[i, 0]) / (2 * dr)
            elif i==1 and j==1:
                div[i, j] = (fx[1, j] - fx[0, j]) / (2 * dr) + (fy[i, 1] - fy[i, 0]) / (2 * dr)
            else:
